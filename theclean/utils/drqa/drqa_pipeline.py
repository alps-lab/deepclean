...
from drqa.reader.vector import batchify
from drqa.reader.data import ReaderDataset, SortedBatchSampler
import drqa.reader as reader
# from drqa import reader
import drqa.tokenizers as tokenizers
...
class DrQA(object):
    ...
    def process(self, query, candidates=None, candidate_doc_scores=None, top_n=1, n_docs=5,
    ...
    def process_batch(self, queries, candidates=None, candidate_doc_scores=None, top_n=1, n_docs=5,
                      return_context=False):
        ...
        if len(queries) == 1:
            names, scores = [], []
            if candidate_doc_scores is not None:
                names.extend(candidate_doc_scores[0][0])
                scores.extend(candidate_doc_scores[0][1])
            if len(names) == 0:
                ret = self.ranker.closest_docs(queries[0], k=n_docs)
                names.extend(ret[0])
                scores.extend(ret[1].tolist())

            ds = (names, scores)
            ranked = [ds]
        else:
            ranked = []
            default_ranked = self.ranker.batch_closest_docs(
                    queries, k=n_docs, num_workers=self.num_workers
                )
            if candidate_doc_scores is not None:
                for i, ds in enumerate(candidate_doc_scores):
                    if len(ds[0]) == 0:
                        ranked.append(default_ranked[i])
                    else:
                        ranked.append(ds)
            else:
                # print(default_ranked)
                ranked.extend(default_ranked)
